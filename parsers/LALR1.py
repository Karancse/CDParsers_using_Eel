...
def generatelalr1(pr2,inp):
    ...
    while(len(st)>0):
        ...
        for i in trz[zx]:
            ...
            for qq in range( 0 , len(sta) ):
                ...
                zqq2=0
                for qqqq3 in sta[qq]:
                    zqq2+=len(sta[qq][qqqq3])
                zqq3=0
                for qqqq3 in st3:
                   zqq3+=len(st3[qqqq3])
                if(zqq2==zqq3):
                    for q4 in list(sta[qq].keys()):
                    
                        q3=0
                        if(q4 not in st3.keys()):
                            q7=1
                            break
                        for q5 in st3[q4]:
                            for qq3 in sta[qq][q4]:
                                if(qq3[0]==q5[0]):
                                    q3=1
                                    break
                        if(q3==0):
    ...
    s="<div class=\"titlespace\"><h1 id=\"title1\">LALR(1)</h1></div><div id=\"outbox1\">"
    ...
    s+="<table id=\"table1\"><caption>LALR(1) Table</caption><tr><th id=\"table1h1\"></th>"
    for i in T:
        s+="<th>"+i+"</th>"
    s+="<th id=\"table1h2\">$</th>"
    for i in NT[1:-1]:
    ...
    if(len(sc)+len(rc)>0):
        ...
        print("The Given Grammer is Not an LALR(1) Grammar.")
        s+="<h4>The Given Grammar is Not an LALR(1) Grammar</h4></div>"
        return s
    s+="<div class=\"lists1\"><ul class=\"list1\"><li>No SR-Conflicts Exists</li></ul><ul class=\"list1\"><li>No RR-Conflicts Exists</li></ul></div></div><h4>The Given Grammar is an LALR(1) Grammar</h4></div>"
